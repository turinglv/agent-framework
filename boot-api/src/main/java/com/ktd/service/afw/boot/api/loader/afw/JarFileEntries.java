...
import com.ktd.service.afw.boot.api.loader.data.RandomAccessData;
...
/**
 * Provides access to entries from a {@link JarFile}. In order to reduce memory consumption entry
 * details are stored using int arrays. The {@code hashCodes} array stores the hash code of the
 * entry name, the {@code centralDirectoryOffsets} provides the offset to the central directory
 * record and {@code positions} provides the original order position of the entry. The arrays are
 * stored in hashCode order so that a binary search can be used to find a name.
 * <p>
 * A typical Spring Boot application will have somewhere in the region of 10,500 entries which
 * should consume about 122K.
 *
 * @author Phillip Webb
