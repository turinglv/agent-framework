...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
  ...
  LocalCache(CacheBuilder<? super K, ? super V> builder,
    ...
    if (evictsBySize()) {
      ...
      for (int i = 0; i < this.segments.length; ++i) {
        this.segments[i] = createSegment(segmentSize, UNSET_INT,
            builder.getStatsCounterSupplier().get());
      }
    }
  }
  ...
  public V merge(K key, V newValue, BiFunction<? super V, ? super V, ? extends V> function) {
    ...
    checkNotNull(function);
    return compute(key,
        (k, oldValue) -> (oldValue == null) ? newValue : function.apply(oldValue, newValue));
  }
    ...
    STRONG {
      ...
      <K, V> ValueReference<K, V> referenceValue(Segment<K, V> segment, ReferenceEntry<K, V> entry,
        ...
        return (weight == 1) ? new SoftValueReference<K, V>(segment.valueReferenceQueue, value,
            entry) : new WeightedSoftValueReference<K, V>(segment.valueReferenceQueue, value, entry,
            weight);
      }
      ...
      <K, V> ValueReference<K, V> referenceValue(Segment<K, V> segment, ReferenceEntry<K, V> entry,
        ...
        return (weight == 1) ? new WeakValueReference<K, V>(segment.valueReferenceQueue, value,
            entry) : new WeightedWeakValueReference<K, V>(segment.valueReferenceQueue, value, entry,
            weight);
      }
  ...
  static class Segment<K, V> extends ReentrantLock {
    ...
    V lockedGetOrLoad(K key, int hash, CacheLoader<? super K, V> loader) throws ExecutionException {
      ...
      try {
        ...
        for (e = first; e != null; e = e.getNext()) {
          K entryKey = e.getKey();
          if (e.getHash() == hash && entryKey != null && map.keyEquivalence.equivalent(key,
              entryKey)) {
     ...
     * is already loading.
     */
    @Nullable LoadingValueReference<K, V> insertLoadingValueReference(final K key, final int hash,
        boolean checkTime) {
      ...
      try {
        ...
        for (e = first; e != null; e = e.getNext()) {
          ...
          if (e.getHash() == hash && entryKey != null && map.keyEquivalence.equivalent(key,
            ...
            ValueReference<K, V> valueReference = e.getValueReference();
            if (valueReference.isLoading() || (checkTime && (now - e.getWriteTime()
                < map.refreshNanos))) {
      ...
      try {
        ...
        for (ReferenceEntry<K, V> e = first; e != null; e = e.getNext()) {
          ...
          if (e.getHash() == hash && entryKey != null && map.keyEquivalence.equivalent(key,
            ...
            ++modCount;
            ReferenceEntry<K, V> newFirst = removeValueFromChain(first, e, entryKey, hash,
                entryValue, valueReference, cause);
            newCount = this.count - 1;
            table.set(index, newFirst);
            this.count = newCount; // write-volatile
            return entryValue;
      ...
      try {
        ...
        for (ReferenceEntry<K, V> e = first; e != null; e = e.getNext()) {
          ...
          if (e.getHash() == hash && entryKey != null && map.keyEquivalence.equivalent(key,
            ...
            if (oldValueReference == valueReference || (entryValue == null
          ...
          for (int i = 0; i < table.length(); ++i) {
            for (ReferenceEntry<K, V> e = table.get(i); e != null; e = e.getNext()) {
              ...
              if (e.getValueReference().isActive()) {
                ...
                enqueueNotification(key, e.getHash(), value, e.getValueReference().getWeight(),
                    cause);
              }
            }
          }
   ...
   * of the queue as part of copyWriteEntry, and (2) the contains method is highly optimized for the
   * current model.
   */
   ...
   */
  static class ManualSerializationProxy<K, V> extends ForwardingCache<K, V> implements
      Serializable {
   ...
   * the proxy must be able to behave as the cache itself.
   */
  static final class LoadingSerializationProxy<K, V> extends
      ManualSerializationProxy<K, V> implements LoadingCache<K, V>, Serializable {
  ...
  static class LocalLoadingCache<K, V> extends LocalManualCache<K, V> implements
      LoadingCache<K, V> {
