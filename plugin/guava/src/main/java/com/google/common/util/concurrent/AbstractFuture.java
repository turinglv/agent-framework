 ...
 * way to set the result of the computation through the protected methods {@link #set(Object)},
 * {@link #setFuture(ListenableFuture)} and {@link #setException(Throwable)}. Subclasses may also
 * override {@link #afterDone()}, which will be invoked automatically when the future completes.
...
public abstract class AbstractFuture<V> extends FluentFuture<V> {
   ...
   * the current thread is interrupted during the call, even if the value is already available.
   *
  ...
  public V get(long timeout, TimeUnit unit)
    ...
    if (isDone()) {
      throw new TimeoutException("Waited " + timeout + " " + Ascii.toLowerCase(unit.toString())
  ...
  public boolean cancel(boolean mayInterruptIfRunning) {
    ...
    if (localValue == null | localValue instanceof SetFuture) {
      ...
      Object valueToSet = GENERATE_CANCELLATION_CAUSES ? new Cancellation(mayInterruptIfRunning,
  ...
  protected @Nullable String pendingToString() {
    ...
    if (localValue instanceof SetFuture) {
      ...
      return "remaining delay=[" + ((ScheduledFuture) this).getDelay(TimeUnit.MILLISECONDS)
          + " ms]";
    }
    return null;
  }
