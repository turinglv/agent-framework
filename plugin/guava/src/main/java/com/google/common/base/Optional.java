 ...
 * T} reference. It allows you to represent "a {@code T} that must be present" and a "a {@code T}
...
public abstract class Optional<T> implements Serializable {
   ...
   * {@code Optional} class; use {@code optionals.stream().filter(Optional::isPresent).map(Optional::get)}
   * instead.
   *
   ...
   *                               false}); depending on this <i>specific</i> exception type (over
   *                               the more general {@link RuntimeException}) is discouraged
   */
